...
async function InvoiceDetailContent({ id }: { id: string }) {
  ...
  return (
    <InvoiceDetailActions invoice={invoice} organization={organization}>
      <div className="flex-1 overflow-y-auto pb-24">
        <div className="px-6 py-5 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
                ...
                <div className="flex justify-end">
                  <div className="w-56 space-y-1.5 text-sm">
                    <div className="flex justify-between text-text-secondary">
                      <span>Subtotal</span>
                      <CurrencyDisplay amount={invoice.subtotal} currency={invoice.currency} className="tabular-nums" />
                    </div>
                    {invoice.discount_amount > 0 && (
                      <div className="flex justify-between text-text-secondary">
                        <span>Discount</span>
                        <span className="tabular-nums text-status-paid-text">
                          - <CurrencyDisplay amount={invoice.discount_amount} currency={invoice.currency} />
                        </span>
                      </div>
                    )}
                    {invoice.tax_amount > 0 && (
                      <div className="flex justify-between text-text-secondary">
                        ...
                        <CurrencyDisplay amount={invoice.tax_amount} currency={invoice.currency} className="tabular-nums" />
                      </div>
                    )}
                    <div className="flex justify-between font-bold text-text-primary pt-2 border-t border-border">
                      <span>Total</span>
                      <CurrencyDisplay amount={invoice.total} currency={invoice.currency} className="tabular-nums" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            {(invoice.notes || invoice.terms) && (
              <div className="bg-surface rounded-lg border border-border p-5 space-y-3">
                {invoice.notes && (
                  <div>
                    <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-1">Notes</p>
                    <p className="text-sm text-text-secondary whitespace-pre-wrap">{invoice.notes}</p>
                  </div>
                )}
                {invoice.terms && (
                  <div>
                    <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-1">Terms</p>
          ...
          <div className="space-y-4">
            {invoice.customer && (
              <div className="bg-surface rounded-lg border border-border p-5">
                <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-3">Bill to</p>
                ...
                {invoice.customer.address_line1 && (
                  <p className="text-sm text-text-secondary mt-1">{invoice.customer.address_line1}</p>
                )}
              </div>
            )}

            <div className="bg-surface rounded-lg border border-border p-5">
              <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">Amount due</p>
              <CurrencyDisplay amount={invoice.total} currency={invoice.currency} className="text-2xl font-bold text-text-primary" />
              {invoice.paid_at && (
            ...
            <div className="bg-surface rounded-lg border border-border p-5">
              <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">Public link</p>
              <p className="text-xs text-text-secondary break-all">/invoice/{invoice.public_id}</p>
            </div>
          </div>
        </div>
      </div>
