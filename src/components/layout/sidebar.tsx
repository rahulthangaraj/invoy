...
export function Sidebar() {
  ...
  return (
    <TooltipProvider>
      ...
      <aside
        className={cn(
          'hidden md:flex flex-col shrink-0 h-screen bg-sidebar-bg border-r border-white/[0.06] sticky top-0 transition-all duration-200',
          collapsed ? 'w-16' : 'w-60',
        )}
      >
        ...
        >
          ...
          {!collapsed && (
            <span className="text-white font-semibold text-sm tracking-tight">Invoy</span>
          )}
          <button
            onClick={toggleCollapsed}
            className={cn(
              'absolute flex items-center justify-center w-5 h-5 rounded-full bg-sidebar-hover text-sidebar-text hover:text-white transition-colors -right-2.5 top-1/2 -translate-y-1/2',
            )}
      ...
      {/* Mobile header bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 flex items-center h-14 px-4 bg-sidebar-bg border-b border-white/[0.06]">
        ...
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="sm" className="text-sidebar-text hover:text-white hover:bg-sidebar-hover px-2">
              <Menu className="w-5 h-5" />
            </Button>
          </SheetTrigger>
          <SheetContent
            side="left"
            className="w-60 p-0 bg-sidebar-bg border-r border-white/[0.06] [&>button]:text-sidebar-text [&>button]:hover:text-white"
          >
